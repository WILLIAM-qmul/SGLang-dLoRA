...
class InstanceManager:
    def __init__(
        ...
        # Calculate LoRA sizes during initialization
    ...
    async def _select_engine_rank_aware(
        ...
        """
        Rank-Aware dispatch with full dLoRA mapping support:
        1. Ensure model has ≥1 engine (same as dLoRA)
        2. DISPATCH_MIG: consider swap candidates (same as dLoRA)
        3. Use RankAwareScheduler for cost scoring within candidate set
        4. Apply swap if chosen engine is a swap candidate
        """
        # ── Step 1: Build candidate set with mapping (needs lock) ────────
        async with self.select_lock:
            self.expected_lora_distribution[model_id] += 1
            await self._update_engine_costs()
            ...
            if len(self.model_engine_mapping[model_id]) == 0:
                ...
                self.model_engine_mapping[model_id] = [engine_id]
                self.engine_model_mapping[engine_id].append(model_id)
                logger.info(f"[RankAware] Added model {model_id} to engine {engine_id}")
                await self._apply_lora_placement_single(engine_id)

            # Primary candidates: engines that already have this model
            candidate_ids: List[int] = list(self.model_engine_mapping[model_id])

            # ★ DISPATCH_MIG swap candidates (NEW for rank-aware)
            sub_engine_model_ids: Dict[int, int] = {}
            if self.migration_type == MigrationType.DISPATCH_MIG:
        ...
        # ── Step 2: Rank-aware scoring (lock released, async HTTP) ───────
        ...
        # ── Step 3: Apply selection + handle swap (re-acquire lock) ──────
        async with self.select_lock:
            # Handle LoRA swap if selected engine is a swap candidate
            if selected_engine in sub_engine_model_ids:
                sub_model_id = sub_engine_model_ids[selected_engine]
                logger.info(
                    f"[RankAware] Substituting model {sub_model_id} with {model_id} "
                    f"on engine {selected_engine}"
                )
                self.engine_model_mapping[selected_engine].remove(sub_model_id)
                self.model_engine_mapping[sub_model_id].remove(selected_engine)
                self.engine_model_mapping[selected_engine].append(model_id)
                self.model_engine_mapping[model_id].append(selected_engine)
                await self._apply_lora_placement_single(selected_engine)
    ...
    async def _select_engine_hybrid(
        ...
        """
        Hybrid dispatch: dLoRA-first, rank-aware only when near SLO.

        Key optimization vs previous design:
        - Phase 1 uses _update_engine_costs() which already fetches engine_num_requests
        - SLO check uses a LIGHTWEIGHT heuristic based on request count × avg_rank,
          NOT an extra HTTP round-trip
        - Only when the heuristic triggers do we fetch /get_rank_aware_stats
        """
        slo_ms = self.rank_aware_scheduler.params.slo_ms
        threshold = self.hybrid_slo_threshold_ratio

        # ── Phase 1: Full dLoRA candidate selection (same as _select_engine_dlora) ──
        async with self.select_lock:
            self.expected_lora_distribution[model_id] += 1
            await self._update_engine_costs()

            # Ensure model has at least one engine
            if len(self.model_engine_mapping[model_id]) == 0:
                engine_id = min(
                    range(self.num_instances),
                    key=lambda eid: self.engine_request_count[eid],
                ...
                self.model_engine_mapping[model_id] = [engine_id]
                self.engine_model_mapping[engine_id].append(model_id)
                logger.info(f"[Hybrid] Added model {model_id} to engine {engine_id}")
                await self._apply_lora_placement_single(engine_id)

            # Build candidate set with dLoRA logic (including DISPATCH_MIG swap)
            candidate_engines: Dict[int, float] = {}
            sub_engine_model_ids: Dict[int, int] = {}
            ...
            # ── Lightweight SLO check (NO extra HTTP) ────────────────────
            # Heuristic: if any candidate engine has enough requests that
            # the decode latency *could* approach SLO, escalate to rank-aware.
            # Use engine_num_requests (already fetched by _update_engine_costs)
            # and average rank to estimate DecPerf.
            req_rank = self._infer_req_rank(model_id)
            need_rank_aware = False

            for eid in candidate_engines:
                n_reqs = self.engine_num_requests.get(eid, 0)
                # Quick estimate: assume all requests have average rank
                avg_rank = req_rank  # conservative: use incoming request's rank
                estimated_predictor = math.sqrt(n_reqs * avg_rank) if n_reqs > 0 else 0.0
                p = self.rank_aware_scheduler.params
                estimated_latency = p.dec_alpha * estimated_predictor + p.dec_beta
                if estimated_latency >= slo_ms * threshold:
                    need_rank_aware = True
                    logger.info(
                        f"[Hybrid] Engine {eid} near SLO: "
                        f"est={estimated_latency:.1f}ms >= "
                        f"threshold={slo_ms * threshold:.1f}ms "
                        f"(n_reqs={n_reqs}). Escalating to rank-aware."
                    )
                    break

            candidate_ids = list(candidate_engines.keys())

        # ── Phase 2: If not near SLO, use dLoRA choice (fast path) ──────
        if not need_rank_aware:
            final_choice = dlora_choice
            logger.debug(
                f"[Hybrid] Below SLO threshold → dLoRA choice: engine {final_choice}"
            )
        else:
            # ── Slow path: fetch rank stats & score by TOPPINGS cost ─────
            # Use cache if fresh enough
            now = time.time()
            if (now - self._rank_states_cache_time) > self._rank_states_cache_ttl:
                states = await self.rank_aware_scheduler._fetch_all(candidate_ids)
                self._rank_states_cache = states
                self._rank_states_cache_time = now
            else:
                states = self._rank_states_cache
            ...
            for eid in candidate_ids:
                ...
                if state is None:
                    from sglang.srt.instances.rank_aware_scheduler import EngineRankState
                    state = EngineRankState.empty(eid)

                w_seq_lens = state.waiting_seq_lens
                if len(w_seq_lens) != len(state.waiting_ranks):
                    default_sl = self.rank_aware_scheduler.default_seq_len
                    w_seq_lens = [
                        w_seq_lens[i] if i < len(w_seq_lens) else default_sl
                ...
                # Blend in dLoRA exec cost as tiebreaker (includes swap penalty)
                dlora_penalty = candidate_engines.get(eid, 0)
                total_cost += dlora_penalty * 0.01  # small weight to break ties
        ...
        logger.debug(
            f"[Hybrid] req {request_id} (model {model_id}, rank {req_rank}) "
            f"→ engine {final_choice} (rank_aware={'YES' if need_rank_aware else 'NO'})"
        )
        return final_choice
    # ── Select engine done──────────────────────────────
    ...
    def _check_migration_needed(self) -> Tuple[bool, Optional[List[int]]]:
        ...
        engine_page_cnt = {
            i: sum(req.num_pages for req in self.reqs_metadata.get(i, []))
    ...
    def _prepare_ilp_problem(self, ilp_engines: List[int]) -> Dict:
        ...
        ilp_reqs_metadata = []
        for engine_id in ilp_engines: 
            for req_metadata in self.reqs_metadata.get(engine_id, []):
                # Remap engine and model IDs
                if req_metadata.model_id not in ilp_model_mapping:
                    continue
                ilp_reqs_metadata.append(
                    RequestMetadata(
                        request_id=req_metadata.request_id,
                        model_id=ilp_model_mapping[req_metadata.model_id],
                        engine_id=ilp_engine_mapping[engine_id],
                        num_pages=req_metadata.num_pages,
                        in_gpu=req_metadata.in_gpu,
                        prompt_length=req_metadata.prompt_length,
                        output_length=req_metadata.output_length,
                    )
                )
        ...
        for engine_id, model_ids in self.engine_model_mapping.items():
            if model_id in ilp_model_mapping:
                    ilp_k = ilp_model_mapping[model_id]
                    ilp_j = ilp_engine_mapping[engine_id]
                    ilp_model_engine_mapping[ilp_k].append(ilp_j)
            for model_id in model_ids: 
                if model_id in ilp_model_mapping:
    ...
    async def _execute_migration(self, migration_plan: Dict, ilp_engines: List[int]):
        """
        Execute migration plan with correct order.
        
        Steps:
        1. Fetch requests from source engines (save state)
        2. Abort requests on source engines (stop execution)
        3. Update LoRA placement from ILP
        4. Adjust LoRA adapters on engines
        5. Insert requests to destination engines (resume execution)
        6. Notify Unified Server
        ...
        total_req_migrations = sum(
            len(reqs) for dsts in req_migration.values() for reqs in dsts.values()
        )
        
        logger.info(
            f"Migration plan:\n"
            f"  LoRA adjustments: {lora_weights}\n"
            f"  Request migrations: {total_req_migrations} requests"
        )
        
        session = await self._get_session()
        
        # ===== Step 1: Fetch requests from source engines =====
        logger. info("=" * 60)
        logger.info("Step 1: Fetching requests from source engines...")
        logger.info("=" * 60)
        ...
        if not fetch_tasks:
            logger.info("No requests to migrate, skipping migration execution")
            return
        ...
        # Validate fetch results and build valid migration list
        valid_migrations = []
        for idx, (src_engine_id, dst_engine_id, request_ids) in enumerate(fetch_info):
            ...
            if not seq_groups:
                logger.warning(
                    f"No seq_groups fetched from engine {src_engine_id} for "
                    f"{len(request_ids)} requests (possibly already completed)"
                )
                continue
            
            valid_migrations. append((src_engine_id, dst_engine_id, request_ids, seq_groups))
            logger.info(
                f"✓ Fetched {len(seq_groups)} seq_groups:  "
                f"Engine {src_engine_id} -> Engine {dst_engine_id}"
            )
        
        if not valid_migrations:
            logger.warning("⚠ No valid seq_groups fetched, aborting migration")
            return
        
        logger.info(f"✓ Successfully fetched {len(valid_migrations)} request groups")
        
        # ===== Step 2: Abort requests on source engines =====
        logger.info("=" * 60)
        logger.info("Step 2: Aborting requests on source engines...")
        logger.info("=" * 60)
        
        abort_tasks = []
        for src_engine_id, dst_engine_id, request_ids, _ in valid_migrations:
        ...
        for idx, (src_engine_id, _, request_ids, _) in enumerate(valid_migrations):
            if idx < len(abort_results):
                result = abort_results[idx]
                if isinstance(result, Exception):
                    logger.error(f"✗ Failed to abort requests on engine {src_engine_id}: {result}")
                elif result:
                    aborted_count = result.get("aborted_count", len(request_ids))
                    logger.info(f"✓ Aborted {aborted_count} requests on Engine {src_engine_id}")
        
        # Give engines time to process aborts
        await asyncio.sleep(0.1)
        
        # ===== Step 3: Update LoRA placement from ILP solution =====
        logger.info("=" * 60)
        logger.info("Step 3: Updating LoRA placement from ILP solution...")
        logger.info("=" * 60)
        
        # Update engine_model_mapping
        for engine_id, model_ids in lora_weights.items():
            old_models = self.engine_model_mapping.get(engine_id, [])
            self.engine_model_mapping[engine_id] = model_ids
            logger.debug(f"Engine {engine_id}:  {old_models} -> {model_ids}")
        
        # Rebuild model_engine_mapping (reverse mapping)
        self.model_engine_mapping = {i: [] for i in range(self.num_models)}
        for engine_id, model_ids in self. engine_model_mapping.items():
            for model_id in model_ids:
                self.model_engine_mapping[model_id].append(engine_id)
        ...
        # Refine placement with greedy algorithm (optional optimization)
        logger.info("Refining LoRA placement with greedy algorithm...")
        try:
            ...
            logger.info(f"✓ Refined engine_model_mapping: {self.engine_model_mapping}")
        except Exception as e:
            logger.error(f"Failed to refine LoRA placement (continuing anyway): {e}", exc_info=True)
        
        # ===== Step 4: Adjust LoRA adapters on engines =====
        logger.info("=" * 60)
        logger.info("Step 4: Syncing LoRA adapters on engines...")
        logger.info("=" * 60)
        
        try:
            await self._execute_lora_adjustment(self. engine_model_mapping)
            logger.info("✓ LoRA adapters synced successfully")
        except Exception as e:
            logger. error(f"LoRA adjustment failed (continuing anyway): {e}", exc_info=True)
        
        # Give engines time to load LoRAs
        await asyncio.sleep(0.2)
        
        # ===== Step 5: Insert requests to destination engines =====
        logger. info("=" * 60)
        logger.info("Step 5: Inserting requests to destination engines...")
        logger.info("=" * 60)
        
        insert_tasks = []
        insert_info = []
        
        for src_engine_id, dst_engine_id, request_ids, seq_groups in valid_migrations:
        ...
        # ===== Step 6: Notify Unified Server =====
        migrations_to_notify = []
        ...
        for idx, (src_engine_id, dst_engine_id, request_ids) in enumerate(insert_info):
            ...
            if inserted_count > 0:
                # Record migrations for notification
                for rid in request_ids:
                    migrations_to_notify.append({
                        "request_id": rid,
                        "old_engine": src_engine_id,
                        "new_engine": dst_engine_id
                    })
                
                successful_migrations += inserted_count
                logger.info(
                    f"✓ Inserted {inserted_count} requests: "
                    f"Engine {src_engine_id} -> Engine {dst_engine_id}"
                )
            else:
                logger. warning(
                    f"✗ Failed to insert requests from engine {src_engine_id} to {dst_engine_id}"
                )
        
        # Notify Unified Server of successful migrations
        if migrations_to_notify:
            logger.info("=" * 60)
            logger.info(f"Step 6: Notifying Unified Server of {len(migrations_to_notify)} migrations...")
            logger.info("=" * 60)
            await self._notify_unified_server_migration(migrations_to_notify)
        
        logger.info("=" * 60)
        logger.info(f"✓ Migration execution complete:  {successful_migrations} requests migrated")
        logger.info("=" * 60)
    ...
    async def _notify_unified_server_migration(
        self,
        migrations: List[Dict[str, Any]]
    ):
        """
        通知 Unified Server 请求已迁移
        ...
        try:
            session = await self._get_session()
            url = f"{self.unified_server_url}/notify_migration"
            ...
            async with session.post(
                ...
                if resp.status == 200:
                    logger. info(f"✓ Notified Unified Server: {len(migrations)} migrations")
                else:
                    error_text = await resp.text()
                    logger.warning(f"Failed to notify Unified Server: {resp.status}, {error_text}")
        
        except Exception as e:
            logger.error(f"Error notifying Unified Server: {e}")
    
    
    async def _execute_request_migration(self, req_migration: Dict[int, Dict[int, List[str]]]):
        """
        Migrate requests between engines. 
        Follows dLoRA's flow: Fetch -> Insert -> Notify -> Abort
        ...
        # Step 3: Notify Unified Server BEFORE aborting
        migrations_to_notify = []
        abort_tasks = []
        ...
        for idx, (src_engine_id, dst_engine_id, request_ids) in enumerate(insert_info):
            ...
            if inserted_count > 0:
                ...
                # prepare abort task
                url = f"{self.instance_urls[src_engine_id]}/abort_requests"
                abort_tasks.append((url, request_ids))
        ...
        # notify Unified Server
        if migrations_to_notify: 
            logger.info(f"Notifying Unified Server of {len(migrations_to_notify)} migrations...")
            await self._notify_unified_server_migration(migrations_to_notify)
            
            # give time for notification to process
            await asyncio.sleep(0.2)
        
        # Step 4: abort requests on source engines
        if abort_tasks:
            logger.info(f"Aborting {len(abort_tasks)} request groups from source engines...")
            abort_coroutines = [
                self._abort_requests(session, url, request_ids)
                for url, request_ids in abort_tasks
            ]
            await asyncio.gather(*abort_coroutines, return_exceptions=True)
    ...
    async def close(self):
        """
        Gracefully shutdown the InstanceManager.
        Clean up resources including HTTP sessions and background tasks.
        """
        logger.info("Shutting down InstanceManager...")
        
        # 1. Stop background migration loop
        if self._running:
            logger.info("Stopping background migration loop...")
            self._running = False
            
            if self. background_loop and not self.background_loop.done():
                self. background_loop.cancel()
                try:
                    await self.background_loop
                except asyncio. CancelledError:
                    logger.info("Background loop cancelled successfully")
                except Exception as e: 
                    logger.error(f"Error stopping background loop: {e}")
        ...
        # 3. Clear request mappings (optional cleanup)
