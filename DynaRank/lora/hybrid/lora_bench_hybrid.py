...
"""
lora_bench_migration.py

This script is designed for benchmarking the SGLang LoRA online inference service, supporting two architectural modes: 
static allocation (sglang) and dynamic load balancing (dLoRA).

Key features include:
- Generate and distribute multi-path requests, supporting random/sequential LoRA allocation or playback based on real-world traces.
- Support multi-instance deployment, with automatic routing of requests to different backend instances.
- Calculate and output multiple performance metrics such as throughput, latency, and token processing rate.
- Support saving test results as JSONL files for subsequent analysis.
- Enable flexible configuration of test scale, request rate, allocation strategy, trace files, etc., via command-line arguments.

Applicable scenarios:
- Compare performance under different LoRA Serving architectures.
- Verify load balancing effects in multi-instance deployment.
- Evaluate service capabilities under trace playback.

Usage:
    python lora_bench_migration.py [arguments]
See `--help` in the command line for detailed parameter descriptions.
"""
...
async def benchmark(
    ...
    if hasattr(args, 'hybrid_scheduling') and args.hybrid_scheduling:
        _routing = "hybrid"
    elif hasattr(args, 'rank_aware_scheduling') and args.rank_aware_scheduling:
        _routing = "rank_aware"
    elif use_unified_server:
        _routing = "least_loaded"
    else:
        _routing = "static"
    ...
    if use_unified_server:
        ...
        print(f"  - Routing Strategy: {_routing}")
    ...
    print("{:<40} {:<10}".format("LoRA assignment strategy:", _routing))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--inference-architecture", type=str, default="sglang",
