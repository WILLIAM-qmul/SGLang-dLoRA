...
def run_unified_server(args: argparse. Namespace):
    ...
    manager = InstanceManager(
        ...
        use_rank_aware_scheduling=args.rank_aware_scheduling,
        # ── Hybrid scheduling (NEW) ───────────────────────────────────────────
        use_hybrid_scheduling=args.hybrid_scheduling,
        hybrid_slo_threshold_ratio=args.hybrid_slo_threshold_ratio,
        # ── Shared rank-aware params ──────────────────────────────────────────
        rank_aware_backend=args.rank_aware_backend,
        rank_aware_avg_resp_len=args.rank_aware_avg_resp_len,
        rank_aware_slo_ms=args.rank_aware_slo_ms,
        rank_aware_default_req_rank=args.rank_aware_default_req_rank,
...
if __name__ == "__main__": 
    ...
    parser.add_argument(
        ...
        default=False,
        help=(
            "Enable hybrid scheduling: dLoRA least-loaded by default, "
            "switches to rank-aware when any candidate engine's estimated "
            "latency approaches the SLO threshold."
        ),
    )
    parser.add_argument(
        "--hybrid-slo-threshold-ratio",
        type=float,
        default=0.7,
        help=(
            "Ratio of SLO at which to trigger rank-aware fallback. "
            "E.g. 0.7 means when estimated latency >= 70%% of SLO. Default: 0.7."
        ),
    )
