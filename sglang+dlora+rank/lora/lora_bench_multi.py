    ...
    async with _create_bench_client_session() as session:  # 使用 bench_serving 的会话工厂（含超时和read_bufsize）
        ...
        if args.base_only:
            ...
            payload = {
                ...
                "lora_num": request_func_input.lora_num,  # 传递 LoRA ID
            }
...
async def benchmark(
    ...
    test_lora_id = 0 if not args.base_only else None
...
def run_benchmark(args_: argparse.Namespace):
    ...
    # Read dataset
    backend = args.backend
    base_model_id = args.model = LORA_PATH["base"]  # 修改变量名
    ...
    return asyncio.run(
        benchmark(
            backend=backend,
            api_url=api_url,
            base_model_id=base_model_id,  # 传递 base_model_id
            tokenizer=tokenizer,
            input_requests=input_requests,
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help="多个实例地址，会自动补全 /generate，例如: http://127.0.0.1:30001 http://127.0.0.1:30002",
    )
    # ==== 新增 trace 控制参数 ====
    ...
    parser.add_argument("--trace-path", type=str, default="/workspace/datasets/maf2/", help="trace 根目录（包含 csv / txt）")
    parser.add_argument("--trace-start-time", type=str, default="0.0.0", help="起始时间 day.hour.min 例如 0.0.0")
    parser.add_argument("--trace-end-time", type=str, default="0.0.60", help="结束时间 day.hour.min 例如 0.0.60")
    parser.add_argument("--trace-interval-minutes", type=int, default=60, help="聚合直方图的时间粒度 (minutes)")
    parser.add_argument("--trace-arrival-distribution", type=str, default="gamma", choices=["gamma", "exponential"], help="用直方图估计的到达分布类型")
    parser.add_argument("--trace-total-rate", type=float, default=4.0, help="整体总到达率 (用于按比例分配各模型速率)")
    parser.add_argument("--trace-cv", type=float, default=1.0, help="Gamma 分布的变异系数 (cv=1 为 Poisson)")
    parser.add_argument("--trace-map-stride", type=int, default=1, help="函数映射到模型的步长")
    parser.add_argument("--trace-need-sort", action="store_true", help="是否按调用总次数排序函数以映射模型")
