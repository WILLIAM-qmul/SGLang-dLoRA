...
async def benchmark(
    ...
    if use_unified_server:
        ...
        # 新增: 追踪实际分发的请求数 (用于 sglang 模式统计)
        request_distribution: Dict[str, int] = defaultdict(int)
    ...
    if use_unified_server:
        try:
            async with aiohttp.ClientSession() as session:
                reset_url = f"{unified_server_url}/reset_manager_stats"
                async with session.post(reset_url) as resp:
                    if resp.status == 200:
                        print("[Benchmark] 已重置 Unified Server 统计数据。")
                    else:
                        print(f"[Benchmark] 重置统计失败: {resp.status}")
        except Exception as e:
            print(f"[Benchmark] 重置统计异常: {e}")
    ...
    if args.use_trace:
        ...
        async for model_id, request in request_generator:
            ...
            if use_unified_server:
                ...
                request_distribution[target_api_url] += 1
    ...
    if use_unified_server:
        ...
        # 🔥 修改: SGLang 模式: 基于真实的请求分发统计
        for i, url in enumerate(backend_instance_urls):
            # 从 request_distribution 中获取该实例URL的真实请求数
            instance_url_with_generate = f"{url}/generate"
            count = request_distribution.get(instance_url_with_generate, 0)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--trace-map-stride", type=int, default=10)
