...
class ModelRunner:
    ...
    def __init__(
        ...
        merge_lora_enabled: bool = True,  # 新增参数
    ):
        # Parse args
        self.merge_lora_enabled = merge_lora_enabled  # 保存参数
    ...
    def initialize(self, min_per_gpu_memory: float):
        ...
        # Init lora
        print("Init lora with LoRAManager")
    ...
    def init_lora_manager(self):
        ...
        print("base_model type:", type(self.model))
        print("base_model repr:", repr(self.model))
        """
        一、如果你用的是 Llama 模型，最终会调用 llama.py 里的 LlamaForCausalLM 及相关类进行推理。

        二、如果你启用了 LoRA（低秩适配），流程如下：
        1. 模型加载时，ModelRunner 会根据参数 enable_lora 调用 init_lora_manager()，创建 LoRAManager，并把底层模型（如 LlamaForCausalLM）传进去。
        2. LoRAManager 会遍历模型的所有可插入 LoRA 的层（如 Linear、QKV、MergedColumnParallelLinear 等），用 get_lora_layer 把这些层替换为带 LoRA 功能的包装层（如 ColumnParallelLinearWithLoRA）。
        3. 推理时，模型的 forward 会走到这些 LoRA 包装层。如果当前 batch 需要 LoRA（如 batch 里指定了 lora_ids），这些层会在原始输出基础上加上 LoRA 的增量（A、B矩阵乘法）。
        4. LoRA 权重管理、加载、卸载都由 LoRAManager 负责，支持多 LoRA 动态切换和批量管理。
        简而言之，LoRA 是通过替换模型的部分线性层为带 LoRA 功能的包装层实现的，推理时自动融合 LoRA 权重，无需手动干预。
        """
        self.lora_manager = LoRAManager(
            ...
            server_args=self.server_args,
            merge_lora_enabled=self.merge_lora_enabled,
        )
