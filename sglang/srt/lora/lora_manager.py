...
class LoRAManager:
    ...
    def load_lora_adapter(self, lora_ref: LoRARef) -> LoRAUpdateOutput:
        ...
        try:
            ...
            """
            总结流程：
            1. load_lora_adapter → load_lora_weights：从磁盘加载权重到内存。
            2. prepare_lora_batch → memory_pool.prepare_lora_batch：准备本 batch 需要的权重。
            3. update_lora_info → set_lora_info：把权重 buffer 传递给每个 LoRA 层。
            4. LoRA 层 forward 时用这些权重参与推理。
            """
        ...
        for layer_id, layer_modules in enumerate(self.lora_modules):
            for module_name, module in layer_modules.items():
                ...
                module.set_lora_info(
                    ...
                    merge=self.merge_lora_enabled,  # 新增：传递merge模式
                )
