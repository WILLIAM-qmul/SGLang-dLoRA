"""
bench_kernel_perf_llama2_dual.py — real inference-path version  
=======================================================  
Simulate full 32-layer forward of Llama-2-7B + SGLang LoRA following real inference path:

- Each layer = 4 kernel calls (matching layers.py):
  1. qkv_proj → backend.run_qkv_lora
     A: (nu, 3r, H), B: (nu, Q+2KV, r)
     output_offset: [0, Q, Q+KV, Q+2KV] (4 elements)
...
TOPPINGS model fit (decode / prefill separately):
...
def load_base_model(model_dir: str) -> Dict[int, Dict[str, torch.Tensor]]:
    """
    Return {layer_idx: {module_name: Tensor(out_dim, in_dim)}} on GPU.  
...
def parse_adapter(
    ...
    """
    Parse PEFT state_dict → (lora_A_store, lora_B_store, covered_modules)
...
# ═════════════════════════════════════════════════════════════════════════════
# WeightPool — 每个 rank 加载全部 ADAPTERS_PER_RANK 个 adapter
# ═════════════════════════════════════════════════════════════════════════════

class WeightPool:
    """
    self.adapters[rank] = List[(lA, lB, covered, path)]
    each rank stores up to ADAPTERS_PER_RANK adapters.

    Access method: get_adapter(rank, slot_idx) → returns the slot_idx-th adapter for that rank (with wrap-around mod) → used for: assigning different adapters to each request in homogeneous batch; same for heterogeneous.
    """
    ...
    def __init__(self, base_model_dir: str, registry: List[Tuple[str, int]], seed: int = 42):
        ...
        self.adapters: Dict[int, List[Tuple]] = {r: [] for r in TARGET_RANKS}

        print("\n─── Loading LoRA adapters (up to 4 per rank) ───────────────")
        for rank in TARGET_RANKS:
            ...
            loaded_list = []
            for path, hint in shuffled:
                if len(loaded_list) >= ADAPTERS_PER_RANK: break
                ...
                try:
                    ...
                    loaded_list.append((lA, lB, covered, path))
            ...
            if not loaded_list:
                print(f"  [WARN] rank={rank}: failed to load any adapter"); continue

            self.adapters[rank] = loaded_list
            print(f"  [INFO] rank={rank:3d}  loaded {len(loaded_list)} adapter(s)")

        print()

    def get_adapter(self, rank: int, slot_idx: int = 0):
        """
        Return the slot_idx-th adapter for that rank (wrapping around via modulo).
        If the rank does not exist, fall back to the nearest rank.
        """
        if rank not in self.adapters or not self.adapters[rank]:
            nearest = min(
                (r for r in self.adapters if self.adapters[r]),
                key=lambda r: abs(r - rank), default=None)
            if nearest is None: return {}, {}, []
            rank = nearest
        lst = self.adapters[rank]
        idx = slot_idx % len(lst)
        lA, lB, covered, _ = lst[idx]
        return lA, lB, covered


# ═════════════════════════════════════════════════════════════════════════════
# LayerBuffers — 32 层 stacked GPU 张量
# ═════════════════════════════════════════════════════════════════════════════

class LayerBuffers:
    """
    `ranks[i]` is the rank of request `i`, `slot_ids[i]` is which adapter slot request `i` uses.

    - Homogeneous `[r,r,r,r]`: only one unique rank `r`, but 4 requests use different adapters:
    - `slot_ids = [0,1,2,3]`
    - result: `num_unique = 4`, `weight_indices = [0,1,2,3]`
    - each unique slot is filled with the corresponding adapter weights.

    - Heterogeneous `[r1,r2,r1,r2]`: unique ranks map to slots as `{r1→slot0, r2→slot0}` (same rank merged to same slot)
    - result: `num_unique = 2`, `weight_indices = [0,1,0,1]`
    - each unique rank uses `slot_idx=0` adapter.
    """
    ...
    def __init__(self, pool: WeightPool, ranks: List[int], device: str,
        ...
        if is_homogeneous:
            # 同构：每个请求是独立的 slot，即使 rank 相同
            # weight_indices = [0,1,2,...,bs-1]，每个 slot 对应不同 adapter
            self.weight_indices = list(range(bs))
            self.num_unique     = bs
            # (slot_idx, rank) 对，对应权重填充
            slot_rank_pairs: List[Tuple[int, int]] = [(i, ranks[i]) for i in range(bs)]
        else:
            # 异构：同 rank 合并为同一 slot（原有逻辑）
            seen: Dict[int, int] = {}
            unique_ranks_list: List[int] = []
            self.weight_indices = []
            for r in ranks:
                if r not in seen:
                    seen[r] = len(seen)
                    unique_ranks_list.append(r)
                self.weight_indices.append(seen[r])
            self.num_unique = len(unique_ranks_list)
            # 每个 unique rank 只用 slot_idx=0
            slot_rank_pairs = [(0, r) for r in unique_ranks_list]

        self.unique_ranks = (ranks if is_homogeneous
                             else [r for r in dict.fromkeys(ranks)])
        self.bs           = bs
        ...
        self.qkv_output_offset = torch.tensor(QKV_OUTPUT_OFFSET, dtype=torch.int32, device=device)
        self.gu_output_offset  = torch.tensor(GU_OUTPUT_OFFSET,  dtype=torch.int32, device=device)
        self.max_qkv_out_dim   = MAX_QKV_OUT_DIM

        # ── 填充权重 ─────────────────────────────────────────────────────
        for ui, (slot_idx, rank) in enumerate(slot_rank_pairs):
            lA, lB, covered = pool.get_adapter(rank, slot_idx=slot_idx)
            r = rank
            ...
            for li in range(NUM_LAYERS):
                ...
                if q_a is not None: self.qkv_A[li][ui, :r,      :] = q_a.to(device, DTYPE)
                if k_a is not None: self.qkv_A[li][ui, r:2*r,   :] = k_a.to(device, DTYPE)
                if v_a is not None: self.qkv_A[li][ui, 2*r:3*r, :] = v_a.to(device, DTYPE)
                ...
                if oa is not None: self.o_A[li][ui, :r, :]  = oa[:r, :Q_OUT_DIM].to(device, DTYPE)
                if ob is not None: self.o_B[li][ui, :, :r]  = ob[:HIDDEN_DIM, :r].to(device, DTYPE)
                ...
                if ga is not None: self.gu_A[li][ui, :r,    :]               = ga[:r, :HIDDEN_DIM].to(device, DTYPE)
                if ua is not None: self.gu_A[li][ui, r:2*r, :]               = ua[:r, :HIDDEN_DIM].to(device, DTYPE)
                if gb is not None: self.gu_B[li][ui, :FFN_DIM,           :r] = gb[:FFN_DIM, :r].to(device, DTYPE)
                if ub is not None: self.gu_B[li][ui, FFN_DIM:2*FFN_DIM,  :r] = ub[:FFN_DIM, :r].to(device, DTYPE)

                da = lA.get("down_proj", [None]*NUM_LAYERS)[li]
                db = lB.get("down_proj", [None]*NUM_LAYERS)[li]
                if da is not None: self.dw_A[li][ui, :r, :] = da[:r, :FFN_DIM].to(device, DTYPE)
                if db is not None: self.dw_B[li][ui, :, :r] = db[:HIDDEN_DIM, :r].to(device, DTYPE)
    ...
    for done, ranks in enumerate(configs, 1):
        ...
        if verbose:
            ...
            print(f"  {done:>4}  {row['batch_size']:>3}  {rs:<28}  "
                  ...
                  f"{row['mean_ms']:>10.4f}  {row['std_ms']:>7.4f}")
    return results
...
def fit_and_explore(results: List[Dict], backend: str, mode: str, top_k: int = 5):
    ...
    Y  = np.array([r["mean_ms"] for r in results], dtype=float)
    n  = len(Y)
    Ym = Y.mean()
