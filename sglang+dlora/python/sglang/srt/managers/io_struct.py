...
class RequestMetadata: 
    ...
    def from_req(cls, req, engine_id:  int, num_blocks: int, in_gpu: bool) -> "RequestMetadata":
        # req 可能是 dict 或带属性的对象，这里统一处理
        def _get(d, k, default=None):
            if isinstance(d, dict):
                return d.get(k, default)
            return getattr(d, k, default)

        rid = _get(req, "rid") or _get(req, "request_id") or ""
        model = _get(req, "model_id", None)
        # 规范化 model_id (lora3 / "3" / None)
        def _to_model_id(val):
            if val is None:
                return 0
        ...
        num_tokens = _get(req, "num_tokens", 0) or 0
        return cls(
            request_id=str(rid),
            model_id=_to_model_id(model),
            engine_id=int(engine_id),
            num_pages=int(num_blocks),
            in_gpu=bool(in_gpu),
            prompt_length=int(num_tokens),
            output_length=0,
        )
...
class EngineStats:
    ...
    engine_id: int
    req_metadata: List[RequestMetadata]  # 强类型
    model_exec_time: Dict[int, Tuple[int, float]]  # model_id -> (count, total_time)
    num_requests: int = 0

    @classmethod
    def from_response(cls, data: Dict, engine_id: int) -> "EngineStats":
        # 1) 请求元数据 -> RequestMetadata[]
        req_list = []
        for r in data.get("req_metadata", []) or []:
            # r: {"rid","model_id","num_blocks","in_gpu","num_tokens",...}
            num_blocks = r.get("num_blocks", 0)
            in_gpu = r.get("in_gpu", False)
            req_list.append(RequestMetadata.from_req(r, engine_id, num_blocks, in_gpu))

        # 2) 执行时间表 -> {int: (count, total_time)}
        met_in = data.get("model_exec_time", {}) or {}
        met_out: Dict[int, Tuple[int, float]] = {}
        def _to_int_model_id(k):
            if isinstance(k, int):
                return k
            if isinstance(k, str):
        ...
        for k, v in met_in.items():
            mid = _to_int_model_id(k)
            if isinstance(v, dict):
                cnt = v.get("count", 0)
                tot = v.get("total_time", 0.0)
        ...
        num_requests = int(data.get("num_requests", len(req_list)) or len(req_list))
        return cls(
            engine_id=int(engine_id),
            req_metadata=req_list,
            model_exec_time=met_out,
