...
class InstanceManager:
    ...
    async def _execute_request_migration(self, req_migration: Dict[int, Dict[int, List[str]]]):
        ...
        for idx, (src_engine_id, dst_engine_id, request_ids) in enumerate(insert_info):
            ...
            if inserted_count > 0:
                # ✨ UPDATE MAPPING IMMEDIATELY - This is critical for auto-reconnection! 
                for request_id in request_ids: 
                    old_engine = self.request_to_engine.get(request_id)
                    self.request_to_engine[request_id] = dst_engine_id
                    logger.debug(
                        f"Updated mapping:  request_id={request_id}, "
                        f"old_engine={old_engine}, new_engine={dst_engine_id}"
                    )
