"""
Unified Engine Manager for dLoRA-style Multi-Instance Serving
Based on dLoRA-artifact but adapted for SGLang's HTTP-based architecture
"""
...
class RequestMetadata:
    """Request metadata for migration scheduling."""
    request_id: str
    model_id: int
    engine_id: int
    num_blocks: int
...
class UnifiedEngineManager:
    """
    Unified Engine Manager for dLoRA-style serving.
    
    Manages multiple SGLang server instances via HTTP APIs,
    ...
    def __init__(
        ...
        self.lora_load_cost = 0.1
        self.merge_speed_ratio = 0.6
        
        # Expected LoRA distribution
    ...
    async def initialize(self):
        """Initialize the manager (must be called in async context)."""
        # Create HTTP session
        ...
        # Get capacity info from instances
        ...
        if self.exec_type != ExecType.REPLICATED:
            ...
            self._initialize_replicated_placement()
        
        print(f"[UnifiedEngineManager] Initialization complete")
        print(f"  - Initial LoRA Placement: {self.instance_to_loras}")

    async def _wait_for_instances(self):
        """Wait for all instances to be ready."""
        print("[UnifiedEngineManager] Waiting for instances to be ready...")
        
        max_retries = 60
        retry_interval = 2
        
        for instance_id, url in enumerate(self.instance_urls):
            for retry in range(max_retries):
                try:
                    async with self.session.get(f"{url}/health", timeout=5) as resp:
                        if resp.status == 200:
                            print(f"  ✓ Instance {instance_id} ({url}) is ready")
                            break
                except Exception as e:
                    if retry == max_retries - 1:
                        raise RuntimeError(f"Instance {instance_id} ({url}) failed to start: {e}")
                    await asyncio.sleep(retry_interval)

    async def _initialize_capacity(self):
        """Initialize capacity info from instances."""
        print("[UnifiedEngineManager] Fetching capacity info from instances...")
        
        for i, url in enumerate(self.instance_urls):
            try:
                async with self.session.get(f"{url}/get_server_info") as resp:
                    info = await resp.json()
                    
                    # Extract capacity info
                    self.num_gpu_blocks[i] = info.get('num_gpu_blocks', 1000)
                    self.num_cpu_blocks[i] = int(self.cpu_swap_space // 512)
                    
                    print(f"  Instance {i}: GPU blocks={self.num_gpu_blocks[i]}, "
                          f"CPU blocks={self.num_cpu_blocks[i]}")
            except Exception as e:
                print(f"  Warning: Failed to get info from instance {i}: {e}")
                # Use default values
                self.num_gpu_blocks[i] = 1000
                self.num_cpu_blocks[i] = int(self.cpu_swap_space // 512)
    ...
    def _initialize_replicated_placement(self):
        """Initialize replicated LoRA placement."""
        num_model_per_group = self.num_loras // self.num_instances
        
        for i in range(self.num_loras):
            instance_id = i // num_model_per_group
            self.lora_to_instances[i] = [instance_id]
        
        for i in range(self.num_instances):
            loras = [i * num_model_per_group + j for j in range(num_model_per_group)]
            self.instance_to_loras[i] = loras
    ...
    def start_background_loop(self) -> None:
        ...
        print("[UnifiedEngineManager] Starting background migration loop...")
    ...
    def stop_background_loop(self) -> None:
        ...
        print("[UnifiedEngineManager] Stopping background migration loop...")
        self.is_running_flag = False
    ...
    async def _run_migration_loop(self):
        ...
        while self.is_running_flag:
            try:
                ...
                print(f"[UnifiedEngineManager] Error in migration loop: {e}")
    ...
    async def select_instance(
        ...
        async with self.select_lock:
            ...
            if len(self.lora_to_instances[lora_id]) == 0:
                ...
                print(f"[UnifiedEngineManager] Adding LoRA {lora_id} to instance {instance_id}")
    ...
    async def _update_instance_metrics(self):
        ...
        for instance_id in range(self.num_instances):
            try:
                ...
                # Get instance load info
                async with self.session.get(f"{url}/get_load") as resp:
                    load_info = await resp.json()
                
                # Extract metrics (adjust based on actual SGLang API)
                req_lora_cnt = {}
                for lora_id in self.instance_to_loras[instance_id]:
                    # Simulate LoRA request counts
                    # In real implementation, this should come from the instance
                ...
                print(f"[UnifiedEngineManager] Error updating metrics for instance {instance_id}: {e}")
    ...
    async def _migration_schedule(self):
        ...
        async with self.select_lock:
            ...
            if not need_migration:
                print("[UnifiedEngineManager] No migration needed")
                return
            ...
            print(f"[UnifiedEngineManager] Migration: Instance {most_req_instance_id} -> {matched_instances}")
    ...
    async def _execute_ilp_migration(
        ...
        # Build LoRA-to-instance mapping
        ...
        # Solve ILP
        print(f"[UnifiedEngineManager] Solving ILP with {len(ilp_reqs_metadata)} requests...")
        ...
        if req_migration_decision is None:
            print("[UnifiedEngineManager] ILP migration failed")
            return
        
        # Apply migration decisions
        print(f"[UnifiedEngineManager] Applying migration decisions...")
        for src_eng_id, decision in req_migration_decision.items():
            ...
            for dst_eng_id, req_ids in decision.items():
                ...
                if req_ids:
                    ...
                    # TODO: Implement actual request migration via API
        ...
        print(f"[UnifiedEngineManager] Migration completed")
