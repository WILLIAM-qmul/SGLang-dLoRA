# File: sglang+dLoRA/benchmark/lora/launch_unified_server.py (Updated)
"""
...
async def generate(request: Request):
    """
    Generate completion with dynamic load balancing and migration.
    """
...
def run_unified_server(args):
    ...
    # # Start background migration loop
    # if migration_type != MigrationType.DISPATCH_ONLY:
    #     asyncio.create_task(manager. start_background_loop())
    # 将后台迁移循环启动移到 FastAPI 的 startup 事件
    @app.on_event("startup")
    async def start_manager_background_loop():
        if migration_type != MigrationType.DISPATCH_ONLY:
            # 此时 event loop 已经运行，可以安全调用
            manager.start_background_loop()
    
    print(f"\n[Unified Server] Starting on port {args.port}")
    print(f"[Unified Server] Routing strategy: Dynamic Load Balancing")
    print(f"[Unified Server] Migration type: {migration_type.name}")
